...
FindText_Gui(cmd, arg1:="")
{
  ...
  if (cmd="Capture"||cmd="CaptureS"||cmd="Capture_NoGui"||cmd="CaptureS_NoGui")
  {
    ...
    if (cors.bind!="")
    {
      ...
      Result:="`nSetTitleMatchMode, 2`nid:=WinExist(""" tt """)"
        . "`nFindText_BindWindow(id" (cors.bind=0 ? "":"," cors.bind)
  ...
  if (cmd="Bind0"||cmd="Bind1"||cmd="Bind2"||cmd="Bind3"||cmd="Bind4")
  {
    ListLines, %lls%
    Critical
    FindText_BindWindow(Bind_ID, bind_mode:=SubStr(cmd,0))
    Hotkey, $*RButton, FindText_RButton_Off, On
    ListLines, % (lls:=A_ListLines=0?"Off":"On")?"Off":"Off"
    CoordMode, Mouse
    KeyWait, RButton
    ...
    ListLines, %lls%
    Hotkey, $*RButton, FindText_RButton_Off, Off
    FindText_BindWindow(0), cors.bind:=bind_mode
    return
  }
...
FindText_BindWindow(bind_id:=0, bind_mode:=0, get_id:=0, get_mode:=0)
{
  local
  static id:=0, mode:=0, oldStyle:=0
  if (get_id)
    return id
  if (get_mode)
    return mode
  if (bind_id)
  {
    id:=bind_id, mode:=bind_mode, oldStyle:=0
    if (mode & 1)
    {
      WinGet, oldStyle, ExStyle, ahk_id %id%
  ...
  {
    if (mode & 1) and (oldStyle)
      WinSet, ExStyle, %oldStyle%, ahk_id %id%
    id:=mode:=oldStyle:=0
  }
}
  ...
  , ByRef zx:="", ByRef zy:="", ByRef zw:="", ByRef zh:="")
{
  ...
  if (hBM) and !(w<1 or h<1)
  {
    ...
    {
      if (mode:=FindText_BindWindow(0,0,0,1))<2
      {
      ...
      else
      {
        ...
        DllCall("PrintWindow", Ptr,id, Ptr,hDC2, "uint",(mode>3)*3)
        DllCall("BitBlt",Ptr,mDC,"int",x-zx,"int",y-zy,"int",w,"int",h
        , Ptr,hDC2, "int",x-wx, "int",y-wy, "uint",0x00CC0020|0x40000000)
...
{
  local
  return (!(bits:=FindText_GetBitsFromScreen(0,0,0,0,0,zx,zy,zw,zh))
    or x<zx or x>zx+zw-1 or y<zy or y>zy+zh-1 or !bits.1)
    ? "0xFFFFFF" : Format("0x{:06X}"
    , NumGet(bits.1+(y-zy)*bits.2+(x-zx)*4,"uint")&0xFFFFFF)
}
